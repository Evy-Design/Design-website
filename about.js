...
/* ---- Hero scene ----
   ...
   overlaps it on screen, the photo visibly wins.

   The camera is orthographic, sized so 1 world unit = 1 CSS pixel,
   with (0,0) at the CENTRE of the canvas.

   Two different compositions, per the brief: WIDE keeps the heading
   dead-fixed up top and the photo travelling down to settle beside a
   flanking two-column bio row (lede right-aligned, detail left-
   aligned). NARROW keeps a fixed photo+heading ROW up top instead and
   has the photo travel down to settle beside the (single-column)
   detail paragraph, with lede above it — both lede and detail push
   out from the centre to align with the heading's own left edge as
   the photo arrives. Either way, only the photo's Y ever moves — the
   text's job is just to push out to its resting edge as the photo
   arrives (see update()).

   The card is sticky/pinned for the WHOLE scroll range (the canvas
   itself, via position: sticky in about.css) — nothing here ever
   pauses partway through: scroll progress maps straight to animation
   progress across the full range, so the whole 220svh reads as one
   continuous scrub, never a "stand still, then catch up" motion. */
(function () {
  function initHeroScene() {
    ...
    var TEXT_Z = 0;
    var GAP_PX = 40; // photo-to-text clearance either side, at the flanking bio row (wide layout)
    ...
    function build() {
      ...
      // never a hand-picked line break baked into the copy.
      ...
      var ledeAlign = isNarrow ? "left" : "right";
      ...
      // offset CENTRE by the push amount instead breaks alignment
      // mid-transition — different-width lines land at different
      // fractional offsets from their own edge, so the column reads as
      // a ragged mess instead of a straight edge that just grows/
      ...
      if (isNarrow) {
        ...
        // picked magic number. This is also what keeps the photo from
        // ever running past the canvas's own bottom edge: the bottom
        // of the bio/photo group is pinned a fixed padding above
        // -vh/2, it can never land past it.
        var topPad = vh * 0.2;
        ...
        // level with the bio row's top edge.
        photo.position.x = 0;
      ...
      // since nothing here actually scrolls. 1.2vh of scroll room +
      // the 1vh the canvas itself occupies while pinned = 220svh total
      // for the section, per the brief. Scroll progress maps straight
      // to animation progress across that WHOLE range (see update()) —
      // no dead zone anywhere in it, so it never reads as "standing
